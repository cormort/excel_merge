const ExcelViewer = (() => {
    ...
    function cacheElements() {
        const mapping = {
            ...
            skipTopRowsInput: 'skip-top-rows-input',
    ...
    function bindEvents() {
        ...
        if (elements.skipTopRowsCheckbox) {
            elements.skipTopRowsCheckbox.addEventListener('change', e => {
                elements.skipTopRowsInput.disabled = !e.target.checked;
            });
        }
        ...
        const handleKeywordEnter = (e) => {
            if (e.key === 'Enter') {
                ...
                if (state.isMergedView) {
                    elements.executeFilterSelectionBtn.click();
                } else {
    ...
    function unselectAllMergedRows() {
        ...
        scope.querySelectorAll('.row-checkbox:checked').forEach(cb => cb.checked = false);
        const headerCheckbox = scope.querySelector('thead input[type="checkbox"]');
        if (headerCheckbox) {
        ...
        try { 
            for (let index = 0; index < validation.files.length; index++) { 
                ...
                for (const sheetName of sheetNames) { 
                    ...
                    // 🌟 取得使用者設定的刪除列數 🌟
                    const skipCount = (elements.skipTopRowsCheckbox && elements.skipTopRowsCheckbox.checked) 
                                      ? parseInt(elements.skipTopRowsInput.value, 10) || 0 
                                      : 0;
