...
const ExcelViewer = (() => {
  ...
  const undoManager = {
    ...
    undoLast() {
      ...
      action.restoreFn();
      this.hideToast();
      // 使用原生 alert 會打斷操作，改用 console 提示或讓畫面變化自然呈現
    ...
    hideToast() {
      el.get('undoToast').classList.remove('show');
    }
  };
  ...
  const el = (() => {
    const ids = {
      ...
      reapplyBanner: 'reapply-banner', reapplySettingsBtn: 'reapply-settings-btn', // [A1]
  ...
  function resetControls() {
    // 移除 closeMergeView 以支援 [A3] 保留狀態，只重置主表
  ...
  function deleteSelectedRows() {
    ...
    if (state.isMergedView) {
      ...
      renderMergedTable();
      el.get('dedupResultPanel').classList.add('hidden'); // [B3] 刪除後隱藏提示
  ...
  function deleteSelectedTables() {
    const selected = Array.from(el.get('displayArea').querySelectorAll('.table-select-checkbox:checked')).map(cb => cb.closest('.table-wrapper'));
  ...
  function deleteColumn(headerToDelete) {
    ...
    const backupData = state.mergedData.map(row => ({...row})); // Deep shallow
  ...
  function executeSmartDeduplication() {
    ...
    Object.entries(groups).forEach(([key, items]) => {
      ...
      items.forEach(item => {
        ...
        if (cb && !cb.checked) { 
          cb.checked = true; 
          item.tr.classList.add('dedup-marked'); // [B3] 視覺高亮
          markedCount++; 
        }
      });
    });
  ...
  function showControls(hasHidden) { el.get('controlPanel').classList.remove('hidden'); el.get('mergeViewBtn').classList.toggle('hidden', state.loadedTables <= 1); }
  ...
  function detectHiddenElements() { return 0; } // simplified
  ...
  function clearAllFiles(silent = false) {
    ...
    updateDropAreaDisplay(); resetControls();
  }
  function sortTablesByFundName() { /* Existing Logics */ }
  function updateColumnSelects(headers) { /* Existing Logics */ }
  ...
  function bindEvents() {
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.key === 'Escape') {
        if (!el.get('dedupModal').classList.contains('hidden')) el.get('dedupModal').classList.add('hidden');
    ...
    el.get('displayArea').addEventListener('click', e => {
      if (e.target.classList.contains('delete-table-btn')) {
        ...
        w.querySelector('.table-select-checkbox').checked = true; // 偷吃步：勾選後呼叫批次刪除
        deleteSelectedTables();
      } else if (e.target.classList.contains('delete-rows-btn')) {
        deleteSelectedRows();
      }
    });
    ...
    el.get('mergeViewContent').addEventListener('click', e => {
      ...
      if (delBtn) { e.stopPropagation(); deleteColumn(delBtn.dataset.header); }
    });
